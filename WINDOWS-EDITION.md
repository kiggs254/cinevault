...
- [x] **Phase 4 — TorBox-only downloads (no qBittorrent/Prowlarr/FlareSolverr).**
  - Search infra: TorBox's free search API (`search-api.torbox.app`, IMDB-keyed,
  ...
  - All search call sites (grabMovie, grabEpisode, grabSeason/findSeasonPack,
    runEpisodeGrab, retryFailed, resourceDownload, reSource, planAndSearch, follows
    scan + backfill) route through `getSearch(cfg)` with tmdbId/kind/season/episode
    threaded; guards use `searchReady(cfg)`. Desktop download path is TorBox→local
    with no qBittorrent fallback. One TorBox key covers search + download.

**Milestone: the whole backend is now Docker-free on desktop.** With
