...
/**
 * Curated public BitTorrent announce trackers. The worker appends these to every
 * download (qBittorrent's "add trackers to new torrents") so it discovers peers
 * it otherwise wouldn't — cutting the "no seeders / stalled" failures.
 *
 * Override or disable with EXTRA_TRACKERS (newline- or comma-separated URLs; set
 * to "none"/"off" to turn the feature off).
 */
...
/** Resolve the tracker list to append to new torrents (env override / disable). */
export function resolveExtraTrackers(): string[] {
  const raw = env.EXTRA_TRACKERS?.trim();
  const source = !raw
    ? DEFAULT_TRACKERS // unset or empty → built-in list
    : /^(none|off|false|0|disabled)$/i.test(raw)
      ? "" // explicit opt-out
      : raw; // custom list
  const list = source
    .split(/[\s,]+/)
    .map((s) => s.trim())
    .filter((s) => /^(https?|udp):\/\/\S+$/i.test(s));
  return [...new Set(list)];
}
